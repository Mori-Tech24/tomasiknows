...
if (isset($_POST['act']) && $_POST['act'] == "book_reservation") {
    ...
    try {
        if ($reservation_datetime < time()) {
            ...
            $checkifhaveReservation->execute(['user_id'=>$userid, 'reservation_date'=>$reservation_date, 'reservation_time'=>$reservation_time]);
            $countcheckifhaveReservation= $checkifhaveReservation->rowCount();

            if($countcheckifhaveReservation > 0) {

                $getStatusofRequest = $dbh->prepare("SELECT reservation_status FROM tbl_reservations WHERE user_id = :user_id
                            AND reservation_date = :reservation_date
                            AND reservation_time = :reservation_time");
                $checkifhaveReservation->execute(['user_id'=>$userid, 'reservation_date'=>$reservation_date, 'reservation_time'=>$reservation_time]);

                $st = $checkifhaveReservation->fetch();

                if($st['reservation_status'] != 0) {
                    $output = array("danger", "Error", "You already have request on this date and time.");

                }else {
                    $output = array("danger", "Error", "There is already have a pending request of reservation on this date and time.");
